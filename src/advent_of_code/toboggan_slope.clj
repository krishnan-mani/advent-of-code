...
(def tree-counts (map #(trees-for-slope % problem-grid) all-slopes))

(defn count-trees-for-example-slope []
  (count-trees-for-slope right-3-down-1 problem-grid))

(defn -main [& args]
  (println "Number of trees encountered (right 3, down 1):" (count-trees-for-example-slope))
